...
export class AuthService {
  ...
  async register(dto: RegisterDto) {
    ...
    if (existing) {
      this.metrics.authRegisterTotal.inc({ status: 'conflict' }); 
    ...
    this.metrics.authRegisterTotal.inc({ status: 'success' }); // ← tambah
    return {
      id: user.id,
  ...
  async login(dto: LoginDto) {
    ...
    if (!user) {
      this.metrics.authLoginTotal.inc({ status: 'failed' }); // ← tambah
    ...
    const payload = { sub: user.id, nik: user.nik, nama: user.nama };
    ...
    this.metrics.authLoginTotal.inc({ status: 'success' }); // ← tambah
    return {
      access_token,
      refresh_token,
      expires_in: 900,
  ...
  async refreshToken(plainRefreshToken: string) {
    ...
    try {
      ...
      this.metrics.authRefreshTotal.inc({ status: 'failed' }); // ← tambah
    ...
    if (!tokenRecord) {
      this.metrics.authRefreshTotal.inc({ status: 'failed' }); // ← tambah
      throw new UnauthorizedException(
        'Refresh token tidak ditemukan atau sudah dicabut',
      );
    }
    ...
    this.metrics.authRefreshTotal.inc({ status: 'success' }); // ← tambah
    return { access_token, expires_in: 900 };
  }

  async logout(user_id: string) {
    await this.refreshTokensService.revokeByUserId(user_id);
    this.metrics.authLogoutTotal.inc(); // ← tambah
  }
